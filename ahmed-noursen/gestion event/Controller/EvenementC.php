...
public function tag()
{
    $sql="select * from event where tag=1";
...
public function ajouterCO()
{
 ...
 function setTag(string $ref) {
      ...
      public function ajouterRATE($ev,$rate)
      {
          $sql="insert into rating values(".$ev.",".$rate.")";
           $db = config::getConnexion();
           try{
               $query = $db->prepare($sql);
      ...
      {
          $sql="update  event set rate=(SELECT AVG(rate) FROM rating where event=reference)";
           $db = config::getConnexion();
           try{
               $query = $db->prepare($sql);
