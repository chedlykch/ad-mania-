...
$listeP = $evenementC->afficherPart();
...
$error = "";
    ...
    $result = $date->format('Y-m-d');
      if($result>$_POST['date']){echo '<script>alert("Verifier votre date début!")</script>';}
      if($result>$_POST['datefin']){echo '<script>alert("Verifier votre date fin!")</script>';}
      else if($_POST['date']>$_POST['datefin']){echo '<script>alert("Verifier les dates!")</script>';}
        else if (
            !empty($_POST["nom"]) && 
            !empty($_POST["theme"]) && 
        ...
        else {echo '<script>alert("Verifier que tous les champs sont remplis!")</script>';
...
<link rel="stylesheet" href="style20.css" type="text/css" media="all" />
<!-- Header -->
<div id="header">
  <div class="shell">
    <!-- Logo + Top Nav -->
    <div id="top">
      ...
      <div id="top-navigation"> <a href="../../gestionComptes/View/logout.php">Log out</a> </div>
    </div>
    <!-- End Logo + Top Nav -->
    <!-- Main Nav -->
    <div id="navigation">
      <ul>
        <li><a href="ajouter1.php"><span>Evenements</span></a></li>
        <li><a href="ajouter2.php"><span>Organisateurs</span></a></li>
      </ul>
    </div>
    <!-- End Main Nav -->
  </div>
    ...
    foreach($listeE as $event){
        ?>
              ...
              <td><?php echo $event['reference']; ?></td>
                ...
                <td>   <textarea name="" id="" cols="5" rows="5" class="table" style="overflow:hidden"><?php echo $event['Description']; ?></textarea></td>
                <td><?php echo $event['organisateur']; ?></td>
                <td><?php echo $event['prix']; ?></td>
                <td><?php echo $event['nb_participant']; ?></td>
...
<div class="table">
          ...
          <table width="100%" border="0" cellspacing="0" cellpadding="0" >
            ...
            <th>Id de l'évenement</th>
              <th>Id du partcipant</th>
              <th>NumTel du participant</th>
            ...
            <?php
  foreach($listeP as $par){
      ?>
            <tr>
            <td><?php echo $par['id_event']; ?></td>
              <td><?php echo $par['id_compte']; ?></td>
              <td><?php echo $par['numtel_compte']; ?></td>
...
function drawChart() {
  ...
  var options = {'title':'My Average Event', 'width':750, 'height':400};
